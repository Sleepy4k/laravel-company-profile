...
export default function Home({
    ...
    const searchFieldChanged = (name: string, value: string) => {
        ...
        router.get(route("rbac.permissions.index"), queryParams);
    };
    ...
    const handleDeletePermission = (e: any) => {
        ...
        // Check if key is not same with permission name
        if (key !== permissionDeleteData?.name) {
            closeModal();
            alert.fire({
                title: "Key is not same",
                text: "Key is not same with permission name, aborting deletion",
                icon: "error",
            });
            return;
        }
        ...
        router.delete(
            route("rbac.permissions.destroy", permissionDeleteData?.uuid || 0),
            {
                onProgress: () => {
                    alert.fire({
                        title: "Please wait...",
                        text: "We are deleting the permission.",
    ...
    return (
        <AuthenticatedLayout
            title={trans("page.rbac.permission.title", "Permissions")}
            header={
                <ResponsiveHeader>
                    <Link
                ...
                <DataTable
                    ...
                    header={
                        <>
                            ...
                            >
                                {trans(
                                    "page.rbac.permission.field.name",
                                    "Name"
                                )}
                            </TableHeading>
                            <TableHeading
                            ...
                            >
                                {trans(
                                    "page.rbac.permission.field.causer",
                                    "Guard Name"
                                )}
                            </TableHeading>
                            <TableHeading
                            ...
                            >
                                {trans(
                                    "page.rbac.permission.field.created_at",
                                    "Created At"
                                )}
                            </TableHeading>
                            <TableHeading
                            ...
                            >
                                {trans(
                                    "page.rbac.permission.field.updated_at",
                                    "Updated At"
                                )}
                            </TableHeading>
                        ...
                        data.data.map((item: any, index: number) => (
                            <tr key={index}>
                                ...
                                <td className="px-3 py-2 text-nowrap">
                                    <PopOver>
                                        {can("rbac.permission.show") && (
                                            <Link
                                                href={route(
                                                    "rbac.permissions.show",
                                                    item.uuid
                                                )}
            ...
            <Modal show={confirmingPermissionDeletion} onClose={closeModal}>
                <form className="p-6" onSubmit={handleDeletePermission}>
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-300">
                        ...
                        {permissionDeleteData?.name} permission?
                    </h2>

                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Once you delete this permission, all of its resources
                        and data will be permanently deleted. Please enter this
                        permission name (<b>{permissionDeleteData?.name}</b>) to
