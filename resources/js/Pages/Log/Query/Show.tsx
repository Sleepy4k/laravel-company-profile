...
export default function Show({ auth, data, queryParams, filename }: PageProps<{ data: any, queryParams: any, filename: string }>) {
    ...
    const handleReload = () => router.get(route('log.query.show', filename), queryParams);
    ...
    return (
        <AuthenticatedLayout
            ...
            header={
                <ResponsiveHeader>
                    <Link href={route('log.query.index')} className='bg-primary-700 lg:py-2 py-1 lg:px-3 px-2 text-white dark:text-gray-800 rounded shadow transition-all dark:bg-white hover:bg-primary-700 dark:hover:bg-white dark:focus:bg-white'>
                        Back
                    </Link>
                </ResponsiveHeader>
            }
