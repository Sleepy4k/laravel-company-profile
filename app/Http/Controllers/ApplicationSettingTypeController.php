<?php

namespace App\Http\Controllers;

use App\Models\ApplicationSettingType;
use App\Http\Requests\StoreApplicationSettingTypeRequest;
use App\Http\Requests\UpdateApplicationSettingTypeRequest;

class ApplicationSettingTypeController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreApplicationSettingTypeRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(ApplicationSettingType $applicationSettingType)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(ApplicationSettingType $applicationSettingType)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateApplicationSettingTypeRequest $request, ApplicationSettingType $applicationSettingType)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(ApplicationSettingType $applicationSettingType)
    {
        //
    }
}
